...
$models = @(
  @{slug='iphone-17'; name='iPhone 17'; family='iPhone 17'; width=2.81; height=5.89; radius=112; layout='dual-vertical'; camera=@{x=64;y=57;width=445;height=535}; cameraLip=@{x=96;y=73;width=302;height=530;radius=145}; lenses=@(@(.403,.325),@(.403,.716)); lensRadius=.236; flash=@(.87,.522); sensor=@(.865,.710); lidar=$null; photoSource='assets/mockups/iphone-17-source.png'; crop=@(284,294,464,990)},
...
foreach($m in $models){
  ...
  if($m.cameraLip){$lipX=[float]$m.cameraLip.x;$lipY=[float]$m.cameraLip.y;$lipW=[float]$m.cameraLip.width;$lipH=[float]$m.cameraLip.height;$lipR=[float]$m.cameraLip.radius;$cameraOuter=Rounded-Path $lipX $lipY $lipW $lipH $lipR;$cameraInner=Rounded-Path ($lipX+16) ($lipY+16) ($lipW-32) ($lipH-32) ($lipR-16)}else{$cameraOuter=Rounded-Path ($cx-28) ($cy-28) ($cw+56) ($ch+56) ([Math]::Min($cw,$ch)*.26);$cameraInner=Rounded-Path ($cx-8) ($cy-8) ($cw+16) ($ch+16) ([Math]::Min($cw,$ch)*.225)};$cameraRim=[System.Drawing.Drawing2D.GraphicsPath]::new([System.Drawing.Drawing2D.FillMode]::Alternate);$cameraRim.AddPath($cameraOuter,$false);$cameraRim.AddPath($cameraInner,$false)
  ...
  $cameraShadowPath=$(if($m.cameraLip){$cameraOuter}else{$camera});$bmp=New-Bitmap;$g=New-Graphics $bmp;$g.DrawPath((New-Pen $(if($m.photoSource){28}else{110}) 19 29 34 18),$phone);$g.DrawPath((New-Pen $(if($m.photoSource){32}else{135}) 9 12 14 16),$cameraShadowPath);$g.DrawPath((New-Pen $(if($m.photoSource){20}else{80}) 12 18 21 24),$inner);$g.Dispose();Save-Layer $bmp (Join-Path $dir 'shadow-overlay.png')

  $cameraConfig=@{layout=$m.layout;lenses=$m.lenses;flash=$m.flash;lidar=$m.lidar};if($m.cameraLip){$cameraConfig.lensRadius=$m.lensRadius;$cameraConfig.sensor=$m.sensor;$cameraConfig.lip=$m.cameraLip}
  $config=[ordered]@{id=$m.slug;displayName=$m.name;family=$m.family;model=$m.name;slug=$m.slug;photorealistic=[bool]$m.photoSource;aspectRatio=[Math]::Round($m.width/$m.height,5);physicalSize=@{width=$m.width;height=$m.height};printArea=@{x=.075;y=.012;width=.85;height=.976};safeArea=@{top=.035;right=.055;bottom=.045;left=.055};cameraExclusion=@{x=$cx/1000;y=$cy/2000;width=$cw/1000;height=$ch/2000};camera=$cameraConfig;scalingRules=@{canvasWidth=1000;canvasHeight=2000;minLayerSize=60;maxLayerWidth=870;maxLayerHeight=1250};previewCrop=@{x=.05;y=0;width=.90;height=1};baseImage="/mockups/$($m.slug)/base.png";maskImage="/mockups/$($m.slug)/print-mask.png";overlayImage="/mockups/$($m.slug)/case-overlay.png";shadowImage="/mockups/$($m.slug)/shadow-overlay.png";assets=@{base='base.png';caseOverlay='case-overlay.png';printMask='print-mask.png';cameraMask='camera-mask.png';highlightOverlay='highlight-overlay.png';shadowOverlay='shadow-overlay.png'}}
