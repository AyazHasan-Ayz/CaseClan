...
export async function generateMetadata({params}:{params:Promise<{id:string}>}){const {id}=await params;const p=products.find(p=>p.id===id);return {title:p?`${p.name} — ${p.clan}`:'Case not found',description:p?`Explore ${p.name} from the ${p.clan} clan. Premium ${p.material.toLowerCase()} case design for flagship phones.`:'',openGraph:{title:p?`${p.name} | CASECLAN`:'CASECLAN',description:p?`${p.clan} clan. ${p.colors[0]}. A considered case for your flagship.`:''}}}
