...
export default async function Page({params}:{params:Promise<{slug:string}>}){const {slug}=await params;if(devices.some(d=>d.slug===slug))return <Shop device={slug}/>;if(slug==='shop'||slug==='new-drops'||slug==='personalized'||slug==='collections'||slug==='ready-designs')return <Shop kind="ready"/>;if(slug==='cart')return <CartPage/>;if(slug==='search')return <SearchPage/>;if(slug==='iphone'||slug==='samsung')return <Shop brand={slug==='iphone'?'iPhone':'Samsung'}/>;if(slug==='custom-upload')return <InfoPages slug="design-your-own"/>;if(slug==='account')return <Account/>;if(slug==='wishlist')return <Wishlist/>;if(slug==='checkout')return <Checkout/>;if(!pages.includes(slug))notFound();return <InfoPages slug={slug}/>}
