...
const pages=['personalized','ready-designs','custom-upload','coming-soon','shop','iphone','samsung','new-drops','devices','clans','collections','our-story','faqs','size-guide','track-order','returns','contact','privacy','wishlist','account','checkout'];
...
export default async function Page({params}:{params:Promise<{slug:string}>}){const {slug}=await params;if(devices.some(d=>d.slug===slug))return <Shop device={slug}/>;if(slug==='shop')return <Shop/>;if(slug==='iphone'||slug==='samsung')return <Shop brand={slug==='iphone'?'iPhone':'Samsung'}/>;if(slug==='new-drops'||slug==='personalized'||slug==='collections')return <Shop kind="personalized"/>;if(slug==='ready-designs')return <Shop kind="ready"/>;if(slug==='custom-upload')return <ProductDetail product={products.find(p=>p.kind==='upload')!} upload/>;if(slug==='account')return <Account/>;if(slug==='wishlist')return <Wishlist/>;if(slug==='checkout')return <Checkout/>;if(!pages.includes(slug))notFound();return <InfoPages slug={slug}/>}
